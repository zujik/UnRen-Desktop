...
#
# Display (Linux): force SDL_VIDEODRIVER=x11 when unset/wayland (KDE + bundled lib/).
...
if [ -f "$LIB/lib/python2.7/site.py" ]; then
    ...
    GAME_LD_PATH="$LIB"
elif [ -f "$LIB/lib/python3.9/site.py" ] || [ -f "$LIB/lib/python3.9/site.pyc" ] || [ -d "$LIB/lib/python3.9/encodings" ]; then
    GAME_PYHOME="$LIB/lib/python3.9"
    GAME_LD_PATH="$LIB"
elif [ -f "$ROOT/lib/python3.9/site.py" ] || [ -f "$ROOT/lib/python3.9/site.pyc" ] || [ -d "$ROOT/lib/python3.9/encodings" ]; then
